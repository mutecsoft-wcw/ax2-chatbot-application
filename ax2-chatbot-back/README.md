...
project_root/
├── app/
│   ├── main.py              # FastAPI 진입점 (App 초기화, CORS 및 미들웨어 설정)
│   ├── core/                # 프로젝트 공통 핵심 설정
│   │   ├── config.py        # YAML 설정 파일 로드 및 환경변수(Pydantic Settings) 관리
│   │   └── security.py      # 인증, 인가 및 보안 관련 유틸리티
│   ├── api/                 # API 엔드포인트 레이어
│   │   └── v1/              # API 버전 관리 (v1)
...
│   │   ├── llm.py           # LLM 모델(OpenAI, Claude 등) 인스턴스 생성
...
└── .env                     # API Key, DB 비밀번호 등 민감 정보 관리
```